...
RSpec.describe RuboCop::Cop::Flexport::EngineApiBoundary do
  ...
  context 'struct api protected engines' do
    let(:config) do
      RuboCop::Config.new(
        'Flexport/EngineApiBoundary' => {
          'StructApiProtectedEngines' => ['MyEngine'],
          'EnginesPath' => 'engines'
        }
      )
    end
    ...
    context 'outbound access' do
        ...
        let(:source) do
          <<~RUBY
            class MyEngine
              def foo
                MainApp::EngineApi::ApiModule.bar
                ^^^^^^^^^^^^^^^^^^ Direct non StructApi access of MainApp::EngineApi is disallowed in this file because it's in the MyEngine engine, which is in the StructApiProtectedEngines list.
              end
            end
          RUBY
        end
          ...
          let(:source) do
            <<~RUBY
              class MyEngine
                def foo
                  OtherEngine::Api::ApiModule.bar
                  ^^^^^^^^^^^ Direct non StructApi access of OtherEngine is disallowed in this file because it's in the MyEngine engine, which is in the StructApiProtectedEngines list.
                end
              end
            RUBY
          end
      ...
      context 'when other engine has StructApi' do
        let(:source) do
          <<~RUBY
            class MyEngine
              def foo
                OtherEngine::Api::StructApi::ApiModule.bar
              end
            end
          RUBY
        end
          ...
          let(:source) do
            <<~RUBY
              class Controller < ApplicationController
                def foo
                  MyEngine::WhitelistedModule.bar
                  ^^^^^^^^ All non StructApi direct access of MyEngine engine disallowed because it is in StructApiProtectedEngines list.
                end
              end
            RUBY
          end
    ...
    context 'when EngineSpecificOverrides defined' do
      ...
      let(:config) do
        RuboCop::Config.new(
          'Flexport/EngineApiBoundary' => {
            'StructApiProtectedEngines' => ['OverrideEngine'],
            'UnprotectedEngines' => ['FooIgnoredEngine'],
            'EnginesPath' => 'engines',
            'EngineSpecificOverrides' => [{
              'Engine' => 'my_engine',
      ...
      context 'when not allowed model' do
        let(:source) do
          <<~RUBY
            OverrideEngine::NotAllowedDelivery.first
            ^^^^^^^^^^^^^^ All non StructApi direct access of OverrideEngine engine disallowed because it is in StructApiProtectedEngines list.
          RUBY
        end
